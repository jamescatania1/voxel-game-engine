...
/**
 * A GUI image object that uses the GUI image atlas. Should be registered with the static AddImage() method after creation.
 */
public class Image {
    public Color color;
    ...
    /**
     * Constructor for a GUI image object.
     * @param color
     *      - the image's tint.
     * @param transform
     *      - the image's transform.
     * @param image
     *      - the GUI image atlas's index. Should be one of ATLASIMAGE_*imagename* defines.
     */
    public Image(Color color, Transform transform, int image){
    ...
    /**
     * Adds the cullTransform to one of the image's cull rect slots, if available. Updates vertex info at end, need not call
     * UpdateVertexInfo() after doing this.
     * @param cullTransform
     *      - the transform whose rect is to be registered.
     * @param clipDirection
     *      - whether to clip the parts of the image that are inside the cullTransform (ClipInside), or to
    ...
    /**
     * Adds the cullTransform as a clip rect for the image, i.e. a mask. Updates vertex info at end, need not call
     * UpdateVertexInfo() after doing this.
     * @param cullTransform
     *      - the transform whose rect is to be registered.
     * @param clipDirection
     ...
     *        clip the parts that are outside the cullTransform (Clipoutside).
     * @param index
     *      - the image's index to store the cull rect index. Within range [0, 2].
     */
    ...
    /**
     * Sets the radii, in pixels, of the rounded corners of the image.
     * @param sw
    ...
    /**
     * Updates the static packed info attribute of the image, i.e. image index, cull information. Call whenever these attributes are changed.
     */
    ...
    public int PackVertex(int texCornerIndex, int attribute){
        if(attribute == 0) return ((texCornerIndex & 0x3) << 30) | (packedVertex.x & 0x3FFFFFFF);
